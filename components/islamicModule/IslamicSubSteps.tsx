...
export default function IslamicSubSteps({ role }: { role: 'parent' | 'teacher' }) {
  ...
  const [allItems, setAllItems] = useState<any[]>([]); // To check for completed videos
  ...
  useEffect(() => {
    ...
    // 2. Fetch Islamic Items (to check completion status)
  ...
  const handlePress = async (subItem: any) => {
    ...
    const q = query(collection(db, "islamic_items"), where("categoryID", "==", subItem.id));
    ...
    if (!snap.empty) {
      ...
      if (lessonData.video) {
        router.push({ 
          ...
          params: { itemId: lessonDoc.id, title: subItem.name } 
        });
      } else {
        router.push({ 
  ...
  const renderCard = ({ item }: any) => {
    // Find if the item linked to this subcategory is completed
    ...
    return (
      <TouchableOpacity style={styles.card} onPress={() => handlePress(item)}>
        <View style={styles.whiteBox}>
          ...
          {/* Removed numberOfLines={1} to allow full text display */}
